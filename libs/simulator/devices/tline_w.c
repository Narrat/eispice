...
struct _devicePrivate {
	...
	complex_ *Wi;	/* Propigation Function */
	complex_ *W;	/* Delayless Propigation Function */
...
static int tLineWInitCharacteristics(devicePrivate_ *p)
{
	...
	complex_ *Yc = p->Yc, *Wi = p->Wi, *W = p->W, *Td = p->Td;
	...
	*/
	for(k = 0; k < K; k++) {
		zos = k*N*N;
		ReturnErrIf(mfuncSqrt(&Wi[zos], N, work, lwork));
	}
	...
	*/
	/* The next opertaion is an inplace operation, but want final result
		to be in Yc */
	memcpy(Yc, Wi, K*N*N*sizeof(complex_));
	...
	for(k = 0; k < K; k++) {
		...
		for(j = 0; j < N; j++) {
			for(i = 0; i < N; i++) {
				xos = k*N*N+j*N+i;
				yos = j*N+i;
				W[xos].r = len*(Td[yos].r*2*M_PI*fk[k] - Wi[xos].r);
				W[xos].i = len*(Td[yos].i*2*M_PI*fk[k] - Wi[xos].i);
			}
		}
...
static int deviceClassLoad(device_ *r)
{
	devicePrivate_ *p;
	complex_ *Yxx, *Yxy;
	int i, N, j, *work, Gis0 = 0, row, col, n, m;
	double yc;
	ReturnErrIf(r == NULL);
	p = r->private;
	ReturnErrIf(p == NULL);
	...
	Yxx = malloc(N*N*sizeof(complex_));	/* Y11 and Y22 */
	ReturnErrIf(Yxx == NULL);

	Yxy = malloc(N*N*sizeof(complex_)); /* -Y12 and -Y21 */
	...
	for(i = 0; i < N; i++) {
		for(j = 0; j < N; j++) {
			if(((*p->G0)[i*N+j] > 1.0E-35) || ((*p->Gd)[i*N+j] > 1.0E-35)) {
				Gis0 = 1;
			}
		}
	}
	...
	if(Gis0 == 1) {
		...
		/* Calculate -[W(0)^2] */
		ReturnErrIf(netlibZGEMM('N', 'N', N, N, N, COMPLEXn1R, p->Wi, N,
				p->Wi, N, COMPLEX0, Yxx, N));
		...
		ReturnErrIf(Yxy == NULL);
		ReturnErrIf(netlibZGEMM('N', 'N', N, N, N, COMPLEXn2R, p->Wi, N,
				p->Yc, N, COMPLEX0, Yxy, N));
		...
		ReturnErrIf(netlibZGEMM('N', 'N', N, N, N, COMPLEXn1R, Yxy, N,
				p->Wi, N, COMPLEX1R, Yxx, N));
		...
		for(i = 0; i < N; i++) {
			for(j = 0; j < N; j++) {
				...
				Yxx[i*N+j].i = 0.0;
				Yxy[i*N+j].r = (*p->R0)[i*N+j];
				Yxy[i*N+j].i = 0.0;
			}
		}

		/* Calculate (1/l)*(1/R) */
		ReturnErrIf(netlibZGESV(N, N, Yxy, N, work, Yxx, N));
		memcpy(Yxy, Yxx, N*N*sizeof(complex_));
	}
...
)
{
	...
	p->Wi = malloc(p->K*p->N*p->N*sizeof(complex_));
	ReturnErrIf(p->Wi == NULL);
